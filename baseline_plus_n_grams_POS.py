...
def vector_baseline6(options, infos):
	...
	stylos = get_stylo(infos["text_line"])
...
def prepare_data(options, json_path):
	...
	for ID, infos in dic_json.items():
		vec_B6 = vector_baseline6(options, infos)
		vec_POS = vector_ngrams_pos(options, nlp, infos["text_line"])
		vec = vec_B6 + vec_POS
		X.append(vec)
...
def vectorize(options):
	...
	mkdirs("results_B6_POS_final/%s/"%(dataset_name))

	# Pour chaque classifieur...
	for name_classif, OBJ in liste_classif:
		print("%s%s%s"%("-"*10,name_classif,"-"*10))
		config = "%s__%s"%('B6_n_grams_POS_',name_classif)
		if options.rel == True:
			config += '__freqRel.txt'
		else:
			config += '.txt'
		out_name ="results_B6_POS_final/%s/%s"%(dataset_name, config)
